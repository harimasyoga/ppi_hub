...
<script type="text/javascript">
  ...
  function setDetailPO() {
    ...
    $.ajax({
        url: '<?php echo base_url('Transaksi/get_edit'); ?>',
        type: 'POST',
        data: {id: id,jenis : "SJ",field:'no_po'},
        dataType: "JSON",
    })
    .done(function(data) {
       $("#no_so").val(data.header[0].no_so);
       ...
       $.each(data.header,function(index, value){
        ...
        $("#table-produk tbody").append(
          `<tr id="itemRow${bucket}">
                ...
                <td>
                    <input type="text" name="qty[${bucket}]" id="qty${bucket}" class="angka form-control" value='${sisa}' onkeyup="hitung()" onchange="hitung()" >
                </td>
                <td id="txt_detail_produk${bucket}">
                    ${detail}
                </td>
